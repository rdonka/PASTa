...
%% Prepare Settings
    ...
    addParameter(p, 'filtertype', defaultparameters.filtertype, @(x) ischar(x) && ismember(x, {'nofilter', 'bandpass', 'highpass', 'lowpass','bandstop'})); % filtertype: input must be one of 4 options
    ...
    if ~strcmp(params.filtertype, 'nofilter') % If no filter is specified, skip all filter parameters
        ...
        if strcmp(params.filtertype, 'bandpass') || strcmp(params.filtertype, 'highpass') || strcmp(params.filtertype, 'bandstop')
            params.highpasscutoff = allparams.highpasscutoff;
        end
        if strcmp(params.filtertype, 'bandpass') || strcmp(params.filtertype, 'lowpass') || strcmp(params.filtertype, 'bandstop')
    ...
    for eachfile = 1:length(data)
        ...
        switch params.filtertype
            ...
            case 'lowpass'
                ...
                [b_low,a_low] = butter(params.filterorder,lowpasscutoffval,'low');
            case 'bandstop'
                freqs = sort([params.highpasscutoff, params.lowpasscutoff]);
                Wn = round(freqs ./ nyquist, 6);
                [z, p, k] = butter(params.filterorder, Wn, 'stop');
                [sos, g] = zp2sos(z, p, k);
            case 'nofilter'
