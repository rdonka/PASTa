function [data] = findSessionTransients_localmin(data,whichstream,whichthreshold,whichfs,preminstartms,preminendms,posttransientms,compoundtransientwindowms,quantificationheight,outputtransientdata)
% FINDSESSIONTRANSIENTS_LOCALMIN  Detects and quantifies transients in a data stream
%                                 for the entire session using the pre-transient 
%                                 baseline window minimum.
%
%   FINDSESSIONTRANSIENTS_LOCALMIN(DATA, WHICHBLTYPE, WHICHSTREAM, WHICHTHRESHOLD, WHICHFS...)
%   analyzes the specified data stream to detect transient events based on
%   the pre-transient local minimum and specified threshold. This
...
%                           - sessiontransients_localmin_<THRESHOLDLABEL>: A structure containing:
...
%
%
% See also: findSessionTransients, findSessionTransients_blmean, findSessionTransients_blmin
%
...
% Import required and optional params into a structure
    ...
    for eachfile = 1:length(data)
        ...
        try
            ...
            data(eachfile).(append('sessiontransients_localmin_',whichthreshold)).params = params;
