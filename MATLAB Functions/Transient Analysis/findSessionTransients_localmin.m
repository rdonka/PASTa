...
disp('SESSION TRANSIENTS: Peak baseline determined by min value in the specified baseline window.')
    ...
    for eachfile = 1:length(data)
        ...
        for eachmax = 1:length(allmaxlocs)
            ...
            currpreminloc = allminlocs(find(allminlocs < allmaxlocs(eachmax),1,'last')); % Index of the baseline window min point in the data stream
            currminval = data(eachfile).(whichstream)(currpreminloc); % Find mean of baseline window
