    ...
    for eachfile = 1:length(data)
        ...
        try
            ...
            if params.outputtransientdata == 1 % OPTIONAL: If outputtransientdata is set to 1, prep data table and structure for individual transient streams
                ...
                transientstreamdatacentered = zeros(length(allmaxlocs), (premaxstart+postmaxend+1));
            end
                ...
                if curramp >= data(eachfile).(thresholdfieldname) % If the current transient amplitude is greater than the threshold, quantify and add it to the table 'transientquantification'
                    ...
                    if params.outputtransientdata == 1 % OPTIONAL: If outputtransientdata is set to 1, add transient data stream
                        ...
                        if (currmaxloc-premaxstart) >= 1 && (currmaxloc+postmaxend) <= length(datastream)
                            ...
                            transientstreamdatacentered(transientcount,:) = datastream(currmaxloc-premaxstart:currmaxloc+postmaxend) - currminval;
                        else
                            ...
                            centeredstreamwithnans = streamwithnans - currminval;
                            transientstreamdata(transientcount,:) = streamwithnans;
                            transientstreamdatacentered(transientcount,:) = centeredstreamwithnans;
                        end
                    end
                end               
            end
            ...
            transientdata(eachfile).params.(mfilename).outputpremaxS = params.outputpremaxS;
            transientdata(eachfile).params.(mfilename).outputpostmaxS = params.outputpostmaxS;
            ...
            if params.outputtransientdata == 1
                ...
                transientdata(eachfile).transientstreamdatacentered = transientstreamdatacentered(1:transientcount,:);
            end
        catch ME
            fprintf('   ERROR: %s\n', ME.message);
        end
