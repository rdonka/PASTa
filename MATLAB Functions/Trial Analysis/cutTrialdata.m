...
%
% REQUIRED INPUTS:
%   DATA                - Struct array; each element represents a session and must
...
%
% OUTPUT:
%   DATA     Struct array; This is the original data structure with added fields for
%            trial signal data and epocs.
%               - data.trial_<streamfieldname>: contains the cut trial data with each 
%                 trial saved as a row.
%
%               - data.trial_timeS: time vector in seconds corresponding to
%                 each sample in the trial data stream.
%
%               - data.trial_<startepocfieldname>: contains indices by trial
%                 for the epoc specified by <startepocfieldname> relative to
%                 the cut trial data streams.
%
%               - data.trial_<endepocfieldname>: contains indices by trial
%                 for the epoc specified by <endepocfieldname> relative to 
%                 the cut trial data streams.
%
%               - data.trial_<epocsfieldnames>: contains indices by trial
%                 for each epoc specified by the optional input <epocsfieldnames> 
%                 relative to the cut trial data streams.
%
% EXAMPLE USAGE:
    ...
    % Cut trial data
        ...
        for eachfile = 1:length(data)
            ...
            for eachtrial = 1:size(data(eachfile).(startepocfieldname),1)
                ...
                data(eachfile).(append('trial_',streamfieldname))(eachtrial,1:currtrialsamples) = data(eachfile).(streamfieldname)(data(eachfile).(startepocfieldname)(eachtrial):data(eachfile).(endepocfieldname)(eachtrial)); % Cut stream
                data(eachfile).(append('trial_',startepocfieldname))(eachtrial,1) = 1; % Adjust start epoc
                data(eachfile).(append('trial_',endepocfieldname))(eachtrial,1) = data(eachfile).(endepocfieldname)(eachtrial) - data(eachfile).(startepocfieldname)(eachtrial); % Adjust end epoc
                
                % Add time vector
                trialidxs = 0:1:(currtrialsamples-1); 
                data(eachfile).(append('trial_timeS'))(eachtrial,1:currtrialsamples) = trialidxs/fs;

                % Adjust additional inputs if specified with optional input 'epocsfieldnames'
                if isempty(params.epocsfieldnames)
                    continue
                else
                    for eachepoc = 1:length(params.epocsfieldnames)
                        epoc = char(params.epocsfieldnames(eachepoc));
                        data(eachfile).(append('trial_',epoc))(eachtrial,1) = data(eachfile).(epoc)(eachtrial)-data(eachfile).(startepocfieldname)(eachtrial)+1; % Add adjusted epoc to data structure
                    end
                end
            end
        end
