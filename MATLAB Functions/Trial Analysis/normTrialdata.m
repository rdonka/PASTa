function [data] = normTrialdata(data,trialdatafieldname,trialBLstartepocfieldname,trialBLendepocfieldname)
...
% Cut trial data
    disp(['NORMALIZING TRIAL DATA: Normalizing ', trialdatafieldname, ' to trial baseline.']);
    ...
    for eachfile = 1:length(data)
        ...
        data(eachfile).(append(trialdatafieldname,'z'))(1:ntrials,1:maxtrialsamples) = NaN;
        ...
        for eachtrial = 1:ntrials
            ...
            currtrialBLsd = std(data(eachfile).(trialdatafieldname)(eachtrial,data(eachfile).(trialBLstartepocfieldname)(eachtrial):data(eachfile).(trialBLendepocfieldname)(eachtrial)),'omitnan');

            data(eachfile).(append(trialdatafieldname,'z'))(eachtrial,1:currtrialsamples) = (data(eachfile).(trialdatafieldname)(eachtrial,:) - currtrialBLmean)/currtrialBLsd; % Z score the trial to the trial baseline
        end
    end
end